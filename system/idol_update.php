...
function dd_return($status, $message, $http_ok = 200, $http_err = 400) {
  http_response_code($status ? $http_ok : $http_err);
  die(json_encode(['msg' => $message], JSON_UNESCAPED_UNICODE));
}
...
if ($q_user->rowCount() < 1) dd_return(false, 'ไม่พบผู้ใช้');
...
if (!is_array($body)) dd_return(false, 'รูปแบบข้อมูลไม่ถูกต้อง');
...
if ($id <= 0) dd_return(false, 'ไม่พบไอดีโปรไฟล์');
if ($name === '' || $nickname === '') dd_return(false, 'กรุณากรอกชื่อจริงและชื่อเล่น');

// ดึงโปรไฟล์ + u_admin (CSV) เพื่อตรวจสิทธิ์
$q_chk = dd_q('SELECT id, u_admin FROM idol WHERE id = ?', [$id]);
if ($q_chk->rowCount() < 1) dd_return(false, 'ไม่พบข้อมูล หรือคุณไม่มีสิทธิ์แก้ไขรายการนี้');
$row_chk = $q_chk->fetch(PDO::FETCH_ASSOC);

// ตรวจสิทธิ์: role=2 ผ่าน, role=1 ต้องอยู่ในรายชื่อ u_admin เดิม (CSV)
if ($role !== "2") {
  $admins = array_filter(array_map('intval', array_map('trim', explode(',', (string)($row_chk['u_admin'] ?? '')))));
  if (!in_array((int)$_SESSION['id'], $admins, true)) {
