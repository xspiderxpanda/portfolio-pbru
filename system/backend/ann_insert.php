<?php
    ...
    if (isset($_SESSION['id'])) {
        ...
        if (
            $_POST['name'] != "" AND $_POST['detail'] != "" 
        ) {
            $q_1 = dd_q('SELECT * FROM users WHERE id = ? AND role = 2 ', [$_SESSION['id']]);
            $plr = $q_1->fetch(PDO::FETCH_ASSOC);
            if ($q_1->rowCount() >= 1) {
                $insert = dd_q("INSERT INTO ann (name,detail,img,link,admin,date) VALUES ( ?,?,?,?,NOW() )",[$_POST['name'],$_POST['detail'],$_POST['img'],$_POST['link'],$plr['username']]);
                if($insert){

                    log_admin_action($_SESSION['id'], "INSERT", "เพิ่มระกาศ");
